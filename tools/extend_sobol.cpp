...
std::vector<PolyInfo> collect_polynomials(int start_dim, int target_dim,
                                           int start_degree) {
    ...
    while ((int)polys.size() < needed) {
        ...
        fprintf(stderr, "    degree %d: expecting ~%llu polys... ",
                deg, (unsigned long long)expected);
        ...
        for (auto p : found) {
            if ((int)polys.size() >= needed) break;
...
void run_local(const std::string& input_file, const std::string& output_file,
               int target_dim, sobol::SearchLevel level,
               int num_threads, int n_proj_check) {
    ...
    for (auto& e : existing)
        if ((int)e.s > max_existing_degree) max_existing_degree = e.s;
    ...
    for (int deg = max_existing_degree; (int)new_polys.size() < needed; ++deg) {
        ...
        for (auto p : candidates) {
            if ((int)new_polys.size() >= needed) break;
            // Skip if already used
            bool used = false;
            for (auto u : used_polys) {
                if (u == p) { used = true; break; }
            }
            if (!used) {
        ...
        fprintf(stderr, "  degree %d: %zu candidates, added %d (%.1fs)\n",
                deg, candidates.size(), added, secs);
    }
    ...
    auto results = sobol::process_batch(
        work, existing_v, n_proj_check, num_threads,
        [](uint32_t done, uint32_t total) {
            fprintf(stderr, "\r  %u / %u (%.1f%%)",
    ...
    for (auto& r : results) {
        if (r.t_val == 0) ++t0_count;
        if (r.t_val > max_t) max_t = r.t_val;
    }
    fprintf(stderr, "\n  Quality: %d/%zu with t=0 (Property A), max t=%d\n",
            t0_count, results.size(), max_t);
...
void run_dispatch(const std::string& input_file, const std::string& outdir,
                  int target_dim, sobol::SearchLevel level, int n_batches) {
    auto existing = sobol::load_joe_kuo(input_file);
    ...
    for (auto& e : existing) {
        if ((int)e.s > max_deg) max_deg = e.s;
    ...
    for (int deg = max_deg; (int)polys.size() < needed; ++deg) {
        for (auto p : gf2::enumerate_primitive(deg)) {
            if ((int)polys.size() >= needed) break;
            bool skip = false;
            for (auto u : used) if (u == p) { skip = true; break; }
            if (!skip) polys.push_back({p, deg});
        }
    }
    ...
    for (int b = 0; b < n_batches; ++b) {
        ...
        fprintf(stderr, "  Wrote %s (%d items, dims %u-%u)\n",
                fname, (int)batch.size(), batch.front().dim, batch.back().dim);
    }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--worker")    mode = WORKER;
        else if (arg == "--help" || arg == "-h") { usage(argv[0]); return 0; }
        else if (arg.rfind("--input=", 0) == 0)      input_file = arg.substr(8);
        else if (arg.rfind("--output=", 0) == 0)     output_file = arg.substr(9);
        else if (arg.rfind("--outdir=", 0) == 0)     outdir = arg.substr(9);
        else if (arg.rfind("--target=", 0) == 0)     target = std::stoi(arg.substr(9));
        else if (arg.rfind("--level=", 0) == 0)      level = std::stoi(arg.substr(8));
        else if (arg.rfind("--threads=", 0) == 0)    threads = std::stoi(arg.substr(10));
        else if (arg.rfind("--batches=", 0) == 0)    batches = std::stoi(arg.substr(10));
        else if (arg.rfind("--proj-check=", 0) == 0) proj_check = std::stoi(arg.substr(13));
    ...
    switch (mode) {
    case LOCAL:
        if (input_file.empty()) { fprintf(stderr, "Error: --input required\n"); return 1; }
