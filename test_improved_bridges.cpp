...
double bridge_Method3_MHRefinement(double r1, double r2, double t1, double tk, double t2,
    ...
    double exp_k1 = std::exp(-params.kappa * dt1);
    double var_uncond =
        r1 * c * exp_k1 * (1.0 - exp_k1) + params.theta * c * (1.0 - exp_k1) * (1.0 - exp_k1);
...
void testImprovedBridges() {
    ...
    std::cout << "Scenario: r1=" << r1 << ", r2=" << r2 << ", tk=" << tk << " (midpoint)\n\n";
    ...
    std::cout << "Exact (Rejection):               " << mean_exact << "   " << var_exact << "   "
              << std_exact << "   " << skew_exact << "\n";
    std::cout << "Method 1 (Simple Drift):         " << mean_m1 << "   " << var_m1 << "   "
              << std_m1 << "   " << skew_m1 << "\n";
    std::cout << "Method 2 (Match Mean):           " << mean_m2 << "   " << var_m2 << "   "
              << std_m2 << "   " << skew_m2 << "\n";
    std::cout << "Method 3 (MH Refinement):        " << mean_m3 << "   " << var_m3 << "   "
              << std_m3 << "   " << skew_m3 << "\n";
    std::cout << "Method 4 (Variance Correction):  " << mean_m4 << "   " << var_m4 << "   "
              << std_m4 << "   " << skew_m4 << "\n";
    std::cout << "Method 5 (Transformed Bridge):   " << mean_m5 << "   " << var_m5 << "   "
              << std_m5 << "   " << skew_m5 << "\n\n";
    ...
    auto print_errors = [&](const char* name, double mean, double var, double skew) {
        std::cout << std::setw(32) << std::left << name << std::setw(12) << std::right
                  << (mean - mean_exact) / mean_exact * 100 << std::setw(12)
                  << (var - var_exact) / var_exact * 100 << std::setw(12)
                  << (skew - skew_exact) / skew_exact * 100 << "\n";
    };
