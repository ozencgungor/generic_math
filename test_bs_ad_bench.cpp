...
DoubleT blackScholesNaive(DoubleT S, DoubleT K, DoubleT sigma,
                          DoubleT r, DoubleT T) {
    using std::exp;
    using std::log;
    using std::sqrt;
...
inline var blackScholesAnalytical(const var& S_v, const var& K_v,
                                  const var& sigma_v, const var& r_v,
                                  const var& T_v) {
    ...
    return stan::math::make_callback_var(
        price,
        [S_v, K_v, sigma_v, r_v, T_v,
         dC_dS, dC_dK, dC_dsigma, dC_dr, dC_dT](auto& vi) {
...
int main() {
    ...
    std::cout << "  " << std::setw(28) << "" << std::setw(14) << "Naive" << std::setw(14) << "Analytical" << std::setw(10) << "Ratio" << "\n";
    std::cout << "  " << std::string(66, '-') << "\n";
    std::cout << "  " << std::setw(28) << "Tape (vari) nodes/call"
              << std::setw(14) << nodes_naive
              << std::setw(14) << nodes_analytical
              << std::setw(10) << std::setprecision(1) << (double)nodes_naive / nodes_analytical << "x\n";
    std::cout << "  " << std::setw(28) << std::setprecision(0) << "Arena bytes/call"
              << std::setw(14) << bytes_per_call_naive
              << std::setw(14) << bytes_per_call_analytical
              << std::setw(10) << std::setprecision(1) << bytes_per_call_naive / bytes_per_call_analytical << "x\n";
    std::cout << "  " << std::setw(28) << std::setprecision(0) << ("Arena for " + std::to_string(MEM_BATCH) + " calls")
              << std::setw(14) << bytes_naive_batch
              << std::setw(14) << bytes_analytical_batch
              << std::setw(10) << std::setprecision(1) << (double)bytes_naive_batch / bytes_analytical_batch << "x\n";

    constexpr int PORTFOLIO_SIZE = 10'000;
    std::cout << "\n  Projected for " << PORTFOLIO_SIZE << "-option portfolio (no recover between options):\n";
    std::cout << "    Naive:      " << std::setprecision(2) << bytes_per_call_naive * PORTFOLIO_SIZE / (1024.0 * 1024.0) << " MB\n";
    std::cout << "    Analytical: " << std::setprecision(2) << bytes_per_call_analytical * PORTFOLIO_SIZE / (1024.0 * 1024.0) << " MB\n";
    ...
    double us_double = std::chrono::duration<double, std::micro>(double_bench_end - double_bench_start).count();
    ...
    std::cout << "  Analytical/Naive speedup: " << std::setprecision(2) << avg_naive / avg_analytical << "x\n\n";

    // ── 4. Portfolio-scale benchmark ──
    constexpr int PORTFOLIO_ITERS = 100;

    std::cout << "── Portfolio (" << PORTFOLIO_SIZE
              << " options x " << PORTFOLIO_ITERS << " iterations) ──\n\n";

    std::vector<double> spots(PORTFOLIO_SIZE), strikes(PORTFOLIO_SIZE),
        vols(PORTFOLIO_SIZE), rates(PORTFOLIO_SIZE), expiries(PORTFOLIO_SIZE);
    ...
    for (int iter = 0; iter < PORTFOLIO_ITERS; ++iter) {
        for (int i = 0; i < PORTFOLIO_SIZE; ++i) {
            volatile double price = blackScholesNaive<double>(
                spots[i], strikes[i], vols[i], rates[i], expiries[i]);
            (void)price;
        }
    }
    auto port_double_end = std::chrono::high_resolution_clock::now();
    double ms_double_port = std::chrono::duration<double, std::milli>(port_double_end - port_double_start).count();

    auto portfolio_bench = [&](auto pricer, const char* label) {
        auto start = std::chrono::high_resolution_clock::now();
        for (int iter = 0; iter < PORTFOLIO_ITERS; ++iter) {
            for (int i = 0; i < PORTFOLIO_SIZE; ++i) {
                var S(spots[i]), K(strikes[i]), sigma(vols[i]),
                    r(rates[i]), T(expiries[i]);
                var price = pricer(S, K, sigma, r, T);
                stan::math::grad(price.vi_);
                stan::math::recover_memory();
            }
    ...
    std::cout << "  " << std::setw(22) << "double (no AD)" << ": "
              << std::setprecision(1) << ms_double_port << " ms  ("
              << std::setprecision(3) << ms_double_port / PORTFOLIO_ITERS << " ms per sweep)  (baseline)\n";
    std::cout << "  " << std::setw(22) << "Naive (autodiff)" << ": "
              << std::setprecision(1) << ms_naive_port << " ms  ("
              << std::setprecision(3) << ms_naive_port / PORTFOLIO_ITERS << " ms per sweep)  "
              << std::setprecision(1) << ms_naive_port / ms_double_port << "x vs double\n";
    std::cout << "  " << std::setw(22) << "Analytical (Greeks)" << ": "
              << std::setprecision(1) << ms_analytical_port << " ms  ("
              << std::setprecision(3) << ms_analytical_port / PORTFOLIO_ITERS << " ms per sweep)  "
              << std::setprecision(1) << ms_analytical_port / ms_double_port << "x vs double\n";
