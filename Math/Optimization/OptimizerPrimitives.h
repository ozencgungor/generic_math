...
struct OptimizerState {
    ...
    double start_time = 0.0; ///< seconds, see Math::nowSeconds()
...
template <typename T>
concept OptimizationScalar = detail::is_optimization_scalar<T>::value;
 ...
 * caller's translation unit, e.g. via Math::detail::valueGrad or hand-rolled
...
inline bool stopFtol(const StopCriteria& criteria, double f, double old_f) {
    return detail::relStop(old_f, f, criteria.ftol_rel, criteria.ftol_abs);
}
...
inline bool stopX(const StopCriteria& criteria, const std::vector<double>& x,
    ...
    if (criteria.xtol_rel > 0.0) {
        const double diff = detail::weightedNorm(x, old_x, criteria.x_weights);
        const double norm = detail::weightedNorm(x, {}, criteria.x_weights);
...
inline bool stopDx(const StopCriteria& criteria, const std::vector<double>& x,
    ...
    if (criteria.xtol_rel > 0.0) {
        const double step = detail::weightedNorm(dx, {}, criteria.x_weights);
        const double norm = detail::weightedNorm(x, {}, criteria.x_weights);
...
protected:
    ...
    std::pair<double, std::vector<double>> evalValueGrad(const F& f,
        ...
        if constexpr (std::is_same_v<DoubleT, double> && ValueGradObjective<F>) {
            ...
            return detail::valueGrad<DoubleT>(f, x);
        }
    }
    ...
    double evalValueGradInto(const F& f, const std::vector<double>& x,
        ...
        if constexpr (std::is_same_v<DoubleT, double> && ValueGradObjective<F>) {
            ...
            return detail::valueGrad<DoubleT>(f, x, theta_scratch, grad_out);
        }
    }
    ...
    void evalConstraint(const G& g, const std::vector<double>& x, std::vector<double>& c,
        ...
        if constexpr (std::is_same_v<DoubleT, double>) {
            ...
            detail::constraintValueJacobian<DoubleT>(g, x, c, jacobian);
        }
    }
