...
inline std::vector<uint64_t> compute_v(uint64_t poly, int s,
...
inline int t_value(const std::vector<uint64_t>& v) {
    ...
    for (int col = W - 1; col >= 0 && rank < n; --col) {
        ...
        // Swap and eliminate
        uint64_t tmp = rows[rank]; rows[rank] = rows[pivot]; rows[pivot] = tmp;
...
inline int resolution_2d(const std::vector<uint64_t>& v1,
                          const std::vector<uint64_t>& v2) {
    int n = std::min({(int)v1.size(), (int)v2.size(), W});
    uint64_t rows[128];
    int total = 0;
    for (int i = 0; i < n; ++i) rows[total++] = v1[i];
    for (int i = 0; i < n; ++i) rows[total++] = v2[i];
...
inline std::vector<Entry> load_joe_kuo(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        iss >> e.dim >> e.s >> e.a;
        e.m.resize(e.s);
        for (uint32_t i = 0; i < e.s; ++i) iss >> e.m[i];
        if (iss) entries.push_back(std::move(e));
    }
    return entries;
}

// Save in Joe-Kuo format
inline void save_joe_kuo(const std::string& path,
