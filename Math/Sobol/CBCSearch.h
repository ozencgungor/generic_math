    ...
    for (int k = 1; k < s; ++k) {
        ...
        for (uint64_t mk = 1; mk <= max_mk; mk += 2) {
            ...
            if (t < best_t) {
                best_t = t;
                best_mk = mk;
                if (t == 0) break;  // can't improve on Property A
            }
        }
...
inline SearchResult search_proj_2d(
    uint64_t poly, int s,
...
inline std::vector<SearchResult> process_batch(
    const std::vector<WorkItem>& items,
    const std::vector<std::vector<uint64_t>>& existing_v = {},
    int n_proj_check = 100,
    int num_threads = 0,
    ...
    auto worker = [&]() {
        for (;;) {
            ...
            case SearchLevel::PROJ_2D:
                sr = search_proj_2d(wi.polynomial, wi.degree,
                                     existing_v, n_proj_check);
                break;
            }
...
inline std::string serialize_work(const std::vector<WorkItem>& items) {
    ...
    for (auto& wi : items) {
        ...
        snprintf(buf, sizeof(buf), "%u %d 0x%llx %d %llu\n",
                 wi.dim, wi.degree, (unsigned long long)wi.polynomial,
                 (int)wi.level, (unsigned long long)wi.seed);
        out += buf;
    }
    return out;
}
...
inline std::vector<WorkItem> deserialize_work(const std::string& data) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (sscanf(line.c_str(), "%u %d 0x%llx %d %llu",
                   &wi.dim, &wi.degree, &poly, &level, &seed) == 5) {
            wi.polynomial = poly;
...
inline std::string serialize_results(const std::vector<SearchResult>& results) {
    ...
    for (auto& sr : results) {
        ...
        snprintf(buf, sizeof(buf), "%u\t%u\t%llu",
                 sr.dim, sr.degree, (unsigned long long)sr.a_encoded);
        out += buf;
        for (auto mi : sr.m) {
