...
private:
    ...
    static T smoothAbs(const T& x) {
        ...
        } else if constexpr (std::is_same_v<T, detail::ProbeDual>) {
            return detail::smoothAbs(x);
        } else {
    ...
    static std::vector<T> computeKrugerDerivatives(size_t n, const std::vector<double>& /*dx*/,
        ...
        for (size_t i = 1; i < n - 1; ++i) {
            ...
            if (smooth) {
                const double blend = detail::sigmoid(pa * pb, kappa);
...
public:
    ...
    computeCoefficientsDual(const std::vector<double>& x, const std::vector<detail::ProbeDual>& y,
        ...
        if (n == 2) {
            ...
            a[0] = (y[1] - y[0]) * inv_dx;
            b[0] = detail::ProbeDual(0.0, n);
            c[0] = detail::ProbeDual(0.0, n);
            return;
        }
        ...
        std::vector<detail::ProbeDual> S(n - 1);
        for (size_t i = 0; i < n - 1; ++i)
            S[i] = (y[i + 1] - y[i]) / dx[i];

        std::vector<detail::ProbeDual> derivatives(n);
        switch (da) {
            case DerivativeApprox::Spline:
