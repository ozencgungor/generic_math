...
private:
    ...
    void calculateCoefficients() {
        ...
        for (size_t i = 0; i < n - 1; ++i) {
            m_a[i] = derivatives[i];
            m_b[i] =
                (DoubleT(3.0) * S[i] - derivatives[i + 1] - DoubleT(2.0) * derivatives[i]) / dx[i];
     ...
     */
    std::vector<DoubleT>
    solveTridiagonal(const std::vector<DoubleT>& lower, // n-1 elements (subdiagonal)
     ...
     */
    void computeSplineDerivatives(std::vector<DoubleT>& deriv, const std::vector<DoubleT>& dx,
     ...
     */
    void computeParabolicDerivatives(std::vector<DoubleT>& deriv, const std::vector<DoubleT>& dx,
                                     const std::vector<DoubleT>& S) const {
        size_t n = this->m_x.size();
        ...
        deriv[n - 1] = ((DoubleT(2.0) * dx[n - 2] + dx[n - 3]) * S[n - 2] - dx[n - 2] * S[n - 3]) /
                       (dx[n - 2] + dx[n - 3]);
    }
     ...
     */
    void computeAkimaDerivatives(std::vector<DoubleT>& deriv, const std::vector<DoubleT>& dx,
                                 const std::vector<DoubleT>& S) const {
        size_t n = this->m_x.size();
        ...
        for (size_t i = 2; i < n - 2; ++i) {
            ...
            if ((this->value(S[i - 2]) == this->value(S[i - 1])) &&
        ...
        // Last point
        w1 = abs_impl(DoubleT(4.0) * S[n - 2] * S[n - 2] * S[n - 3] -
                      DoubleT(2.0) * S[n - 2] * S[n - 3]);
     ...
     */
    void computeKrugerDerivatives(std::vector<DoubleT>& deriv, const std::vector<DoubleT>& dx,
                                  const std::vector<DoubleT>& S) const {
        size_t n = this->m_x.size();
     ...
     */
    void computeHarmonicDerivatives(std::vector<DoubleT>& deriv, const std::vector<DoubleT>& dx,
                                    const std::vector<DoubleT>& S) const {
        size_t n = this->m_x.size();
