...
public:
     ...
     */
    GaussLobattoIntegrator(double absAccuracy, size_t maxIterations, double relAccuracy = 0.0,
                           bool useConvergenceEstimate = true)
        : Integrator<DoubleT>(absAccuracy, maxIterations), m_relAccuracy(relAccuracy),
          m_useConvergenceEstimate(useConvergenceEstimate) {}

    /**
     * @brief Integrate f from a to b.
     *
     * double: the adaptive 6-way subdivision implemented below. var and
...
private:
    ...
    double calculateAbsTolerance(const FunctionType& f, DoubleT a, DoubleT b) const {
        ...
        DoubleT acc =
            h * (DoubleT(0.0158271919734801831) * (y1 + y13) +
                 ...
                 DoubleT(0.2249264653333395270) * (f5 + f6) + DoubleT(0.2426110719014077338) * y7);

        this->increaseNumberOfEvaluations(13);

        double acc_val = value(acc);
        if (acc_val == 0.0 && (value(f1) != 0.0 || value(f2) != 0.0 || value(f3) != 0.0 ||
                               value(f4) != 0.0 || value(f5) != 0.0 || value(f6) != 0.0)) {
        ...
        if (m_useConvergenceEstimate) {
            ...
            DoubleT integral1 =
                (h / DoubleT(1470.0)) * (DoubleT(77.0) * (y1 + y13) + DoubleT(432.0) * (y3 + y11) +
                                         DoubleT(625.0) * (y5 + y9) + DoubleT(672.0) * y7);
     ...
     */
    DoubleT adaptiveGaussLobattoStep(const FunctionType& f, DoubleT a, DoubleT b, DoubleT fa,
                                     DoubleT fb, double acc) const {
        ...
        DoubleT integral1 =
            (h / DoubleT(1470.0)) * (DoubleT(77.0) * (fa + fb) + DoubleT(432.0) * (fmll + fmrr) +
                                     DoubleT(625.0) * (fml + fmr) + DoubleT(672.0) * fm);
