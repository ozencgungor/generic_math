 ...
 *   quantape::math::LinearInterpolation<double> interp(x, y);
 *   double value = interp(1.5);  // Interpolate at x=1.5
 *
 *   // Cubic spline interpolation (natural spline)
 *   quantape::math::CubicInterpolation<double> spline(x, y);  // Defaults to Spline method
 *   double spline_value = spline(1.5);
 *   double derivative = spline.derivative(1.5);
 *
 *   // 2D Bicubic interpolation
 *   std::vector<std::vector<double>> z = {{...}, {...}, {...}};
 *   quantape::math::BicubicInterpolation<double> bicubic(x, y, z);  // Defaults to Spline
 ...
 *   quantape::math::CubicInterpolation<ADVariableT> ad_spline(x_ad, y_ad);
