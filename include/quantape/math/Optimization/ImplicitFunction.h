...
void iftUnconstrained(const F2& f2, const std::vector<double>& x_hat, const std::vector<double>& m,
    ...
    std::vector<quantape::math::detail::fvar> theta, xv, mv;
    std::vector<double> e, col, H, G;
    quantape::math::detail::denseHessian(fx, x_hat, H, theta, e, col);
    quantape::math::detail::mixedHessian(f2, x_hat, m, G, xv, mv);
    ...
    Eigen::MatrixXd X;
    quantape::math::detail::solvePositiveDefinite(Hm, RHS, X, options, out);
    for (std::size_t i = 0; i < n; ++i) {
        for (std::size_t j = 0; j < M; ++j) {
...
void iftKkt(const F2& f2, const G2& g2, const H2& h2, const Bounds& bounds,
    ...
    if constexpr (has_g) {
        ...
        quantape::math::detail::constraintValueJacobian<quantape::math::detail::var>(gx, x_hat, c,
                                                                                     Jg);
    ...
    if constexpr (has_h) {
        ...
        quantape::math::detail::constraintValueJacobian<quantape::math::detail::var>(hx, x_hat, ch,
                                                                                     Jh);
    ...
    quantape::math::detail::denseHessian(lagr, x_hat, HL, theta, e, col);
    quantape::math::detail::mixedHessian(f2, x_hat, m, G, xv, mv);
