...
public:
    ...
    OptimizeResult minimizeImpl(const F& f, OptimizerState& state) const {
        ...
        while (true) {
            ...
            const double gd = quantape::math::detail::dotProduct(state.grad, direction);
            const double norm_g = quantape::math::detail::euclideanNorm(state.grad);
            const double norm_d = quantape::math::detail::euclideanNorm(direction);
