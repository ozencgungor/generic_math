...
inline double phi(double x) { return std::exp(-0.5 * x * x) / std::sqrt(2.0 * M_PI); }
...
Greeks compute(double S, double sigma, double r, double K, double T) {
    ...
    g.dRho_dr   = -K * T * T * disc * Nd2
                + K * T * disc * phi(d2) * sqrtT / sigma;
    return g;
}
...
struct bs_vari : public stan::math::vari_value<double> {
    ...
    bs_vari(double val, var S, var sigma, var r,
            double delta, double vega, double rho)
        : vari_value<double>(val),
          S_vi_(S.vi_), sigma_vi_(sigma.vi_), r_vi_(r.vi_),
          delta_(delta), vega_(vega), rho_(rho) {}
...
struct greek_vari : public stan::math::vari_value<double> {
    ...
    greek_vari(double val, var S, var sigma, var r,
               double dS, double dsigma, double dr)
        : vari_value<double>(val),
          S_vi_(S.vi_), sigma_vi_(sigma.vi_), r_vi_(r.vi_),
          dS_(dS), dsigma_(dsigma), dr_(dr) {}
...
fvar<var> price(fvar<var> S, fvar<var> sigma, fvar<var> r, double K, double T) {
    ...
    var delta_var(new greek_vari(g.delta, Sv, sv, rv,
                                g.gamma, g.vanna, g.dDelta_dr));
    var vega_var(new greek_vari(g.vega, Sv, sv, rv,
                                g.vanna, g.volga, g.dVega_dr));
    var rho_var(new greek_vari(g.rho, Sv, sv, rv,
                               g.dDelta_dr, g.dVega_dr, g.dRho_dr));
...
void compute(double S0, double sig0, double r0, double K, double T,
             double& price, Eigen::VectorXd& grad, Eigen::MatrixXd& H) {
    double sqrtT = std::sqrt(T);
...
struct FunctorA {
    ...
    auto operator()(const Vec& v) const { return approach_a::price(v(0), v(1), v(2), K, T); }
};

struct FunctorB {
    double K, T;
    template <typename Vec>
    auto operator()(const Vec& v) const { return approach_b::price(v(0), v(1), v(2), K, T); }
};
...
void print_result(const char* label, double fx, const Eigen::VectorXd& grad,
    ...
    std::cout << "    Grad  = [" << std::setprecision(4)
...
int main() {
    ...
    double t_a = bench_us([&]() {
        double f_; Eigen::VectorXd g_(3); Eigen::MatrixXd H_(3, 3);
