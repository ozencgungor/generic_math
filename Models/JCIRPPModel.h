...
struct JCIRPPParams {
    ...
    JCIRPPParams(double x0, double kappa, double theta, double sigma, double jumpIntensity,
                 double jumpMean, const std::vector<double>& shifts = {});
};
...
public:
    ...
    void update(JCIRPPState& current, const JCIRPPState& previous, size_t stepIndex, double dt,
                const std::vector<double>& dW, unsigned int seed) const;
...
double calculateSurvivalProbability(const std::map<int, JCIRPPState>& path, int fromDay, int toDay);
