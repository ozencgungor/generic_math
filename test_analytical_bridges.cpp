...
double bridge_x_mm_v1_buggy(double r1, double r2, double t1, double tk, double t2, const CIRParams& params) {
    double dt1 = tk - t1;
    if (dt1 < 1e-8) return r1;
    double dt2 = t2 - tk;
    if (dt2 < 1e-8) return r2;
...
double bridge_x_mm_v2_corrected(double r1, double r2, double t1, double tk, double t2, const CIRParams& params, bool use_tv_correction) {
    double dt1 = tk - t1;
    if (dt1 < 1e-8) return r1;
    double dt2 = t2 - tk;
    if (dt2 < 1e-8) return r2;
    ...
    if (use_tv_correction) {
        ...
        if (M_b > 1e-8 && s > 1e-8 && (var_1k + var_k2_approx) > 1e-12) {
            double T_V = (var_1k * var_1k * var_k2_approx + var_1k * var_k2_approx * var_k2_approx) / std::pow(var_1k + var_k2_approx, 2);
    ...
    double var_factor = (k > 1e-8) ? ((r1 / k) * (exp_k_dt1 - e_2kdt1) + (modified_params.theta / (2.0 * k)) * std::pow(1.0 - exp_k_dt1, 2)) : 0.0;
...
std::vector<MethodResult> run_full_scenario_test(const TestCase& test_case) {
    ...
    std::cout << "  Params: k=" << test_case.params.kappa << ", th=" << test_case.params.theta << ", s=" << test_case.params.sigma << std::endl;
    std::cout << "  Values: r1=" << test_case.r1 << ", r2=" << test_case.r2 << std::endl;
    std::cout << "  Time:   t1=" << test_case.t1 << ", tk=" << test_case.tk << ", t2=" << test_case.t2 << std::endl;
    ...
    for(int i=0; i<N_SAMPLES; ++i) {
        rng.seed(1000 + i);
        samples_exact.push_back(sampleCIRBridge_Exact(test_case.r1, test_case.r2, test_case.t1, test_case.tk, test_case.t2, test_case.params));
    }
    ...
    for (const auto& method_name : method_names) {
        ...
        for (int i = 0; i < N_SAMPLES; ++i) {
            ...
            if (method_name == "v1_buggy") {
                samples_approx.push_back(bridge_x_mm_v1_buggy(test_case.r1, test_case.r2, test_case.t1, test_case.tk, test_case.t2, test_case.params));
            } else if (method_name == "v2_corrected") {
                samples_approx.push_back(bridge_x_mm_v2_corrected(test_case.r1, test_case.r2, test_case.t1, test_case.tk, test_case.t2, test_case.params, false));
            } else if (method_name == "v2_Tv") {
                samples_approx.push_back(bridge_x_mm_v2_corrected(test_case.r1, test_case.r2, test_case.t1, test_case.tk, test_case.t2, test_case.params, true));
            }
        }
        ...
        result.skew_err = (std::abs(skew_exact) > 1e-9) ? (skew_approx - skew_exact) / skew_exact * 100 : 0.0;
    ...
    auto print_errors = [&](const MethodResult& res) {
        std::cout << "  " << std::setw(25) << std::left << res.method_name
                  << std::setw(12) << std::right << res.mean_err
                  << std::setw(12) << std::right << res.var_err
...
int main() {
    try {
        ...
        struct TimeVariation { std::string name; double tk; double t2; };
        std::vector<TimeVariation> time_variations = {
            {" (Midpoint)", 0.5, 1.0},
        ...
        std::map<std::string, std::vector<double>> aggregate_mean_errors, aggregate_var_errors, aggregate_skew_errors;
        ...
        std::cout << "\n\n========================================================" << std::endl;
        std::cout << "           AGGREGATE ERROR ACROSS ALL " << test_cases.size() << " SCENARIOS" << std::endl;
        std::cout << "========================================================" << std::endl;
        std::cout << "Method                    Avg Abs Mean Err  Avg Abs Var Err   Avg Abs Skew Err\n";
        std::cout << "--------------------------------------------------------------------------------\n";

        std::vector<std::string> method_names = {"v1_buggy", "v2_corrected", "v2_Tv"};
        for (const auto& method : method_names) {
            double avg_mean_err = std::accumulate(aggregate_mean_errors[method].begin(), aggregate_mean_errors[method].end(), 0.0) / aggregate_mean_errors[method].size();
            double avg_var_err = std::accumulate(aggregate_var_errors[method].begin(), aggregate_var_errors[method].end(), 0.0) / aggregate_var_errors[method].size();
            double avg_skew_err = std::accumulate(aggregate_skew_errors[method].begin(), aggregate_skew_errors[method].end(), 0.0) / aggregate_skew_errors[method].size();

            std::cout << std::setw(25) << std::left << method
                      << std::setw(18) << std::right << avg_mean_err << " %"
                      << std::setw(18) << std::right << avg_var_err << " %"
