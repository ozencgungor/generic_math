...
inline D1D2 d1d2(double S, double sigma, double r) {
    ...
    double d1 = (std::log(S / K_FIX) + (r + 0.5 * sigma * sigma) * T_FIX) / (sigma * sqrtT);
...
struct BSNaiveFunctor {
    ...
    T operator()(const Eigen::Matrix<T, Eigen::Dynamic, 1>& theta) const {
        ...
        T d1 = (stan::math::log(S / K_FIX) + (r + sigma * sigma / 2.0) * T_FIX) / sigSqrtT;
        T d2 = d1 - sigSqrtT;
...
void hessian_reverse_on_greeks(double S0, double s0, double r0, double& fx, Eigen::Vector3d& g,
    ...
    // ONE tape for all three Greeks (d1/d2 computed once, shared by the
    // rows); three reverse sweeps with adjoint zeroing between them (Stan's
    // chain() skips any vari with a nonzero adjoint, so the sweeps must be
    // isolated — accumulation tricks do not work across grad() calls).
...
// ═══════════════════════════════════════════════════════════════════════════
// APPROACH 2b: REVERSE-ON-GREEKS, CALLBACK TAPE (hand-written reverse)
//
// The 2nd-order Greeks are known analytically; instead of differentiating
// the Greek EXPRESSIONS through a var tape (approach 2), each Greek is a
// make_callback_var whose callback encodes its Hessian row. The tape is
// 6 varis (3 leaves + 3 callbacks); each row is one short chain walk.
// ═══════════════════════════════════════════════════════════════════════════

void hessian_callback_greeks(double S0, double s0, double r0, double& fx, Eigen::Vector3d& g,
                             Eigen::Matrix3d& H) {
    fx = bs::price(S0, s0, r0);
    auto g1 = bs::greeks1(S0, s0, r0);
    ...
    var S(S0), sigma(s0), r(r0);
    stan::math::vari* Sv = S.vi_;
    stan::math::vari* sigmav = sigma.vi_;
    stan::math::vari* rv = r.vi_;

    var delta_cb = stan::math::make_callback_var(g1.delta, [Sv, sigmav, rv, g2](auto& vi) {
        const double a = vi.adj();
        Sv->adj_ += a * g2.gamma;
        sigmav->adj_ += a * g2.vanna;
        rv->adj_ += a * g2.dDelta_dr;
    });
    var vega_cb = stan::math::make_callback_var(g1.vega, [Sv, sigmav, rv, g2](auto& vi) {
        const double a = vi.adj();
        Sv->adj_ += a * g2.vanna;
        sigmav->adj_ += a * g2.volga;
        rv->adj_ += a * g2.dVega_dr;
    });
    var rho_cb = stan::math::make_callback_var(g1.rho, [Sv, sigmav, rv, g2](auto& vi) {
        const double a = vi.adj();
        Sv->adj_ += a * g2.dDelta_dr;
        sigmav->adj_ += a * g2.dVega_dr;
        rv->adj_ += a * g2.dRho_dr;
    });

    stan::math::grad(delta_cb.vi_);
    H(0, 0) = S.adj();
    H(0, 1) = sigma.adj();
    H(0, 2) = r.adj();
    stan::math::set_zero_all_adjoints();

    stan::math::grad(vega_cb.vi_);
    H(1, 0) = S.adj();
    H(1, 1) = sigma.adj();
    H(1, 2) = r.adj();
    stan::math::set_zero_all_adjoints();

    stan::math::grad(rho_cb.vi_);
    H(2, 0) = S.adj();
    H(2, 1) = sigma.adj();
    H(2, 2) = r.adj();
...
struct BSNestedFunctor {
    ...
    T operator()(const Eigen::Matrix<T, Eigen::Dynamic, 1>& theta) const {
        if constexpr (std::is_same_v<T, fvar<var>>) {
            ...
            T sqrtT = stan::math::sqrt(T(T_FIX));
            T d1 =
                (stan::math::log(S / K_FIX) + (r + sigma * sigma / 2.0) * T_FIX) / (sigma * sqrtT);
            T d2 = d1 - sigma * sqrtT;
    ...
    fvar<var> compute_nested(const Eigen::Matrix<fvar<var>, Eigen::Dynamic, 1>& theta) const {
        ...
        var delta_var =
            stan::math::make_callback_var(g1.delta, [Sv, sv, rv, gamma = g2.gamma, vanna = g2.vanna,
                                                     dDdr = g2.dDelta_dr](auto& vi) {
                double a = vi.adj();
                Sv.adj() += a * gamma;
                sv.adj() += a * vanna;
                rv.adj() += a * dDdr;
        ...
        var vega_var =
            stan::math::make_callback_var(g1.vega, [Sv, sv, rv, vanna = g2.vanna, volga = g2.volga,
                                                    dVdr = g2.dVega_dr](auto& vi) {
                double a = vi.adj();
        ...
        var rho_var = stan::math::make_callback_var(
            g1.rho,
            [Sv, sv, rv, dRdS = g2.dDelta_dr, dRds = g2.dVega_dr, dRdr = g2.dRho_dr](auto& vi) {
...
void hessian_full_analytical(double S0, double s0, double r0, double& fx, Eigen::Vector3d& g,
                             Eigen::Matrix3d& H) {
    fx = bs::price(S0, s0, r0);
    auto g1 = bs::greeks1(S0, s0, r0);
    auto g2 = bs::greeks2(S0, s0, r0);
    ...
    H << g2.gamma, g2.vanna, g2.dDelta_dr, g2.vanna, g2.volga, g2.dVega_dr, g2.dDelta_dr,
        g2.dVega_dr, g2.dRho_dr;
}
...
struct IRGammaFunctor {
    ...
    T operator()(const Eigen::Matrix<T, Eigen::Dynamic, 1>& theta) const {
        ...
        T sqrtT = stan::math::sqrt(T(T_FIX));
        T d1 =
            (stan::math::log(fwd / K_FIX) + (rate + T(s0) * T(s0) / 2.0) * T_FIX) / (T(s0) * sqrtT);
        T d2 = d1 - T(s0) * sqrtT;
...
int main() {
    ...
    std::cout << "── Correctness (S=" << S0 << ", σ=" << s0 << ", r=" << r0 << ", K=" << K_FIX
              << ", T=" << T_FIX << ") ──\n\n";
    ...
    // Approach 2b: reverse-on-greeks, callback tape
    double fx2b;
    Eigen::Vector3d g2b;
    Eigen::Matrix3d H2b;
    hessian_callback_greeks(S0, s0, r0, fx2b, g2b, H2b);
    ...
    std::cout << "    Callback Greeks:   "
              << (Eigen::MatrixXd(H2b) - Eigen::MatrixXd(H4)).cwiseAbs().maxCoeff() << "\n";
    std::cout << "    Nested analytical: " << (H3 - Eigen::MatrixXd(H4)).cwiseAbs().maxCoeff()
              << "\n";

    // Also verify gradient
    std::cout << std::fixed << "\n  Gradient: [Delta, Vega, Rho]\n";
    std::cout << "    Naive:      [" << std::setprecision(8) << g1(0) << ", " << g1(1) << ", "
              << g1(2) << "]\n";
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            xmm(i) -= eps;
            xmm(j) -= eps;
            auto f = [](const Eigen::Vector3d& v) { return bs::price(v(0), v(1), v(2)); };
    ...
    auto t_callback = bench_us(
        [&]() {
            double fx_;
            Eigen::Vector3d g_;
            Eigen::Matrix3d H_;
            hessian_callback_greeks(S0, s0, r0, fx_, g_, H_);
        },
        N);
    ...
    auto row = [&](const char* name, double t) {
        std::cout << "  " << std::setw(30) << std::left << name << std::right << std::setw(10)
    ...
    row("2b. Callback Greeks", t_callback);
    row("3. Nested analytical", t_nested);
    row("4. Full analytical (dbl)", t_anal);

    std::cout << "\n  Full analytical is " << std::setprecision(0) << t_anal * 1000.0
              << " ns/call — pure arithmetic, no AD overhead.\n";
    ...
    double ir_gamma = g2_vals.gamma * dFwd_dr * dFwd_dr + g1_vals.delta * d2Fwd_dr2;

    std::cout << "  Fwd = " << Fwd << ", dFwd/dr = " << dFwd_dr << ", d²Fwd/dr² = " << d2Fwd_dr2
              << "\n";
    ...
    std::cout << "           = " << std::setprecision(4) << g2_vals.gamma << " × "
              << dFwd_dr * dFwd_dr << " + " << g1_vals.delta << " × " << d2Fwd_dr2 << "\n";
    ...
    std::cout << "\n  Chain rule (analytical):  " << std::setprecision(1) << t_chain * 1000.0
              << " ns\n";
    std::cout << "  Full AD (fvar<var>):      " << std::setprecision(3) << t_ad_chain << " us ("
              << std::setprecision(0) << t_ad_chain * 1000.0 << " ns)\n";
    std::cout << "  Speedup:                  " << std::setprecision(1) << t_ad_chain / t_chain
              << "x\n";
