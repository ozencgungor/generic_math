...
inline double fast_log_refined(double x) {
    ...
    double log_m =
        f * (1.0 + f * (-0.5 + f * (0.33333333333333333 +
...
inline var std_exp_analytical(const var& x) {
    ...
    return stan::math::make_callback_var(val, [x, val](auto& vi) { x.adj() += vi.adj() * val; });
}
...
inline var std_log_analytical(const var& x) {
    ...
    return stan::math::make_callback_var(val, [x, xv](auto& vi) { x.adj() += vi.adj() / xv; });
}
...
void printRow(const char* name, BenchResult dbl, BenchResult ad, AccuracyStats acc,
              std::size_t nodes) {
    std::cout << "  " << std::setw(26) << name << std::setw(10) << std::setprecision(1)
              << dbl.per_call_ns << std::setw(10) << std::setprecision(1) << ad.per_call_ns
...
int main() {
    ...
    std::cout << "  " << std::setw(26) << "Method" << std::setw(10) << "dbl ns" << std::setw(10)
              << "AD ns" << std::setw(10) << "AD/dbl" << std::setw(14) << "Max Rel Err"
              << std::setw(8) << "Nodes" << "\n";
    ...
    // Double timing
    auto dbl_std = benchNs(
        [](double x) {
            volatile double r = std::exp(x);
            (void)r;
        },
        exp_inputs, ITERS);
    auto dbl_schr = benchNs(
        [](double x) {
            volatile double r = fast_exp_schraudolph(x);
            (void)r;
        },
        exp_inputs, ITERS);
    auto dbl_ref = benchNs(
        [](double x) {
            volatile double r = fast_exp_refined(x);
            (void)r;
        },
        exp_inputs, ITERS);
    ...
    // LOG BENCHMARK
    // ═══════════════════════════════════════════════════════════════
    std::cout << "\n── log(x), x ∈ [0.01, 100], " << NPTS << " points x " << ITERS
              << " iters ──\n\n";
    ...
    auto ldbl_std = benchNs(
        [](double x) {
            volatile double r = std::log(x);
            (void)r;
        },
        log_inputs, ITERS);
    auto ldbl_schr = benchNs(
        [](double x) {
            volatile double r = fast_log_schraudolph(x);
            (void)r;
        },
        log_inputs, ITERS);
    auto ldbl_ref = benchNs(
        [](double x) {
            volatile double r = fast_log_refined(x);
            (void)r;
        },
        log_inputs, ITERS);
    ...
    auto cad_naive = benchNsAD([&](const var& x) { return stan::math::exp(a_param * x + b_param); },
                               compound_inputs, ITERS);
    auto cnodes_naive =
        countNodes([&](const var& x) { return stan::math::exp(a_param * x + b_param); });

    // Analytical: compute value in double, push adjoint manually
    // d/dx exp(a*x + b) = a * exp(a*x + b)
    auto cad_anal = benchNsAD(
        [&](const var& x) -> var {
            double xv = x.val();
            double val = std::exp(a_param * xv + b_param);
            double dval_dx = a_param * val;
            return stan::math::make_callback_var(
                val, [x, dval_dx](auto& vi) { x.adj() += vi.adj() * dval_dx; });
        },
        compound_inputs, ITERS);
    ...
    std::cout << "  " << std::setw(26) << "Method" << std::setw(10) << "dbl ns" << std::setw(10)
              << "AD ns" << std::setw(10) << "AD/dbl" << std::setw(8) << "Nodes" << "\n";
    ...
    std::cout << "  Analytical adjoint: " << cnodes_anal
              << " tape node, derivative = a*exp(a*x+b)\n";
    ...
    std::cout << std::fixed;
    std::cout << "  BS double (std::exp/log): " << std::setprecision(1) << bs_std_time.per_call_ns
              << " ns\n";
    std::cout << "  BS double (fast exp/log): " << bs_fast_time.per_call_ns << " ns\n";
    std::cout << "  Speedup: " << std::setprecision(2)
              << bs_std_time.per_call_ns / bs_fast_time.per_call_ns << "x\n";
