...
void testStopCriteria() {
    ...
    CHECK(quantape::math::stopFtol(criteria, 1.0 + 1e-9, 1.0));
    CHECK(!quantape::math::stopFtol(criteria, 1.1, 1.0));
    // function value: absolute
    criteria = {};
    criteria.ftol_abs = 1e-8;
    CHECK(quantape::math::stopFtol(criteria, 1.0 + 1e-9, 1.0));
    CHECK(!quantape::math::stopFtol(criteria, 1.0 + 1e-7, 1.0));
    // zero crossing
    criteria = {};
    criteria.ftol_rel = 1e-8;
    CHECK(quantape::math::stopFtol(criteria, 0.0, 0.0));
    ...
    CHECK(!quantape::math::stopFtol(criteria, 0.0, huge));

    // iterate: relative and absolute
    criteria = {};
    criteria.xtol_rel = 1e-6;
    const std::vector<double> x{1.0, 2.0};
    CHECK(quantape::math::stopX(criteria, x, std::vector<double>{1.0 + 1e-9, 2.0}));
    CHECK(!quantape::math::stopX(criteria, x, std::vector<double>{1.0 + 1e-3, 2.0}));
    criteria = {};
    criteria.xtol_abs = 1e-8;
    CHECK(quantape::math::stopX(criteria, x, std::vector<double>{1.0 + 1e-9, 2.0 - 1e-9}));
    CHECK(!quantape::math::stopX(criteria, x, std::vector<double>{1.0, 2.0 + 1e-7}));

    // step
    criteria = {};
    criteria.xtol_abs = 1e-8;
    CHECK(quantape::math::stopDx(criteria, x, std::vector<double>{1e-9, -1e-9}));
    CHECK(!quantape::math::stopDx(criteria, x, std::vector<double>{1e-9, 1e-7}));

    // gradient
    criteria = {};
    criteria.grad_tol = 1e-8;
    CHECK(quantape::math::stopGrad(criteria, std::vector<double>{1e-9, -1e-9}));
    CHECK(!quantape::math::stopGrad(criteria, std::vector<double>{1e-9, -1e-7}));
    criteria.grad_tol = 0.0;
    CHECK(!quantape::math::stopGrad(criteria, std::vector<double>{0.0, 0.0}));

    // budgets
    criteria = {};
    criteria.maxeval = 5;
    CHECK(quantape::math::stopEvals(criteria, 5));
    CHECK(!quantape::math::stopEvals(criteria, 4));
    criteria = {};
    CHECK(!quantape::math::stopTime(criteria, quantape::math::nowSeconds()));
    criteria.maxtime = 1e-6;
    const double start = quantape::math::nowSeconds() - 1.0;
    CHECK(quantape::math::stopTime(criteria, start));
    CHECK(std::string(quantape::math::to_string(
...
void testLbfgs() {
    ...
    // 1-D quadratic: the first Wolfe step lands exactly on the minimum
    {
        ...
        CHECK(optimizer.minimize(Quadratic1D{}, x) ==
    ...
    // 3-D quadratic: fast convergence to the analytic minimizer
    {
        ...
        CHECK(optimizer.minimize(q, x, state) ==
              quantape::math::OptimizeResult::GradientTolReached);
        for (std::size_t i = 0; i < x.size(); ++i) {
    ...
    // Rosenbrock: classic curved-valley test at var
    {
        ...
        CHECK(optimizer.minimize(Rosenbrock{}, x) ==
              quantape::math::OptimizeResult::GradientTolReached);
    ...
    // `fvar<var>` backend drives the same loop through the value reverse path
    {
        ...
        std::vector<double> x = start;
        CHECK(optimizer.minimize(q, x) == quantape::math::OptimizeResult::GradientTolReached);
    ...
    // maxeval exit
    {
        ...
        quantape::math::LBFGS<var> optimizer(criteria);
        std::vector<double> x = start;
        CHECK(optimizer.minimize(q, x) == quantape::math::OptimizeResult::MaxEvalReached);
    }
    // ftol exit (gradient stop disabled, one pass suffices)
    {
        ...
        quantape::math::LBFGS<var> optimizer(criteria);
        std::vector<double> x = start;
        CHECK(optimizer.minimize(q, x) == quantape::math::OptimizeResult::FtolReached);
    }
    // xtol exit (gradient stop disabled, one pass suffices)
    {
        ...
        quantape::math::LBFGS<var> optimizer(criteria);
        std::vector<double> x = start;
        CHECK(optimizer.minimize(q, x) == quantape::math::OptimizeResult::XtolReached);
    }
    // Small memory: ring buffer wraps (m = 2 < n = 3) and still converges
    {
        ...
        std::vector<double> x = start;
        CHECK(optimizer.minimize(ill, x) == quantape::math::OptimizeResult::GradientTolReached);
...
void testConstraintHelpers() {
    ...
    checkClose("max violation", quantape::math::maxViolation({-1.0, 0.25, 0.0}), 0.25, 1e-15);
    checkClose("l1 penalty", quantape::math::l1Penalty({0.5, -1.0, 0.0}, {2.0, -0.5}), 3.0, 1e-15);
}
...
void testSlSqp() {
    ...
    // Equality-constrained quadratic: min ||x-(2,2)||^2 s.t. x0+x1=2 -> (1,1)
    {
        ...
        CHECK(result == quantape::math::OptimizeResult::Success);
    ...
    // Infeasible system: x0 <= 0 and x0 >= 1
    {
        ...
        CHECK(result == quantape::math::OptimizeResult::Infeasible);
    }
...
void testTNewton() {
    ...
    // 2-D Rosenbrock at var
    {
        ...
        std::vector<double> x{-1.2, 1.0};
        quantape::math::OptimizerState state;
        CHECK(solver.minimize(Rosenbrock{}, x, state) ==
              quantape::math::OptimizeResult::GradientTolReached);
    ...
    // 3-D quadratic: exact Newton directions, fast convergence
    {
        ...
        quantape::math::OptimizerState state;
        CHECK(solver.minimize(q, x, state) == quantape::math::OptimizeResult::GradientTolReached);
        for (std::size_t i = 0; i < x.size(); ++i) {
    ...
    // `fvar<var>` backend drives the same path
    {
        ...
        std::vector<double> x{-1.2, 1.0};
        CHECK(solver.minimize(Rosenbrock{}, x) ==
              quantape::math::OptimizeResult::GradientTolReached);
    ...
    // maxeval exit
    {
        ...
        std::vector<double> x{-1.2, 1.0};
        CHECK(solver.minimize(Rosenbrock{}, x) == quantape::math::OptimizeResult::MaxEvalReached);
    }
...
void testBasePlumbing() {
    ...
    // double backend: value only, no gradient
    {
        ...
        std::vector<double> x = start;
        quantape::math::OptimizerState state;
        CHECK(probe.minimize(q, x, state) == quantape::math::OptimizeResult::Success);
    ...
    // stopval exit
    {
        ...
        SteepestDescent<var> optimizer(criteria, 0.1);
        std::vector<double> x = start;
        CHECK(optimizer.minimize(q, x) == quantape::math::OptimizeResult::StopvalReached);
    }
    // xtol exit
    {
        quantape::math::StopCriteria criteria;
