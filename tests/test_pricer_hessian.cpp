...
private:
    ...
    D1D2 compute(double S, double sigma, double r) const {
        ...
        // F n(d1) = K n(d2): one exp instead of two
        double nd1 = phi(d1);
        double nd2 = (S / K) * nd1;
        return {d1, d2, nd1, nd2, Phi(d1), Phi(d2), std::exp(-r * T), sqrtT};
    }
    ...
    // leaves — exactly what the column-by-column hessian loop needs.
    // 2 callbacks + 0 arithmetic nodes (the 3-callback fan-out is equivalent
    // but costs 3 callback allocations + 5 var nodes per call).
    fvar<var> priceFvarVar(fvar<var> S, fvar<var> sigma, fvar<var> r) const {
        ...
        const double wS = Sd.val(), ws = sd.val(), wr = rd.val();
        const double tval = delta * wS + vega * ws + rho_g * wr;

        stan::math::vari* pS = Sv.vi_;
        stan::math::vari* ps = sv.vi_;
        stan::math::vari* pr = rv.vi_;

        var tangent = stan::math::make_callback_var(
            tval, [pS, ps, pr, wS, ws, wr, gamma_, vanna_, dDdr, volga_, dVdr, dRdr](auto& vi) {
                const double a = vi.adj();
                pS->adj_ += a * (wS * gamma_ + ws * vanna_ + wr * dDdr);
                ps->adj_ += a * (wS * vanna_ + ws * volga_ + wr * dVdr);
                pr->adj_ += a * (wS * dDdr + ws * dVdr + wr * dRdr);
            });
